...
class BuildABoxSection extends Component {
  ...
  #layoutHTML({
    ...
    return `
      <div class="bab-modal" data-bab-root>
        <div class="bab-modal__media">
          ${roastBadge ? `<div class="bab-modal__roast-badge">${escapeHtml(roastBadge)}</div>` : ''}
   ...
   * @returns {string}
   */
  #roastBadgeFromCard(ctx) {
    const card = document.querySelector(`product-card a[href^="/products/${ctx.handle}"]`)?.closest('product-card');
    if (!card) return '';
    // Look for a small "ROAST" label rendered in the card hover-label slot.
...
if (!document.getElementById(STYLE_ID)) {
  ...
  style.textContent = `
    ...
    */
    ...
    .bab-modal__title {
      font-family: 'Founders Grotesk', 'FoundersGrotesk', system-ui, sans-serif;
      font-weight: 600;
      font-size: 24px;
      line-height: 1;
      letter-spacing: 0;
